...
import { runProposeRisks } from "@/lib/ai/actions/proposeRisks";
import { mapAssistActionError } from "@/lib/ai/assist/routeErrors";

// AI Assist trigger — PROPOSE_RISKS (Section 4).
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await runProposeRisks({ initiativeId: id, userId: user.id, organizationId: user.organizationId });
